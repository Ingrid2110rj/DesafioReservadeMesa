...
Cypress.Commands.add('entrega', () => {
  ...
  cy.get(':nth-child(8) > input', { timeout: 12000 }).type('999999999');
...
Cypress.Commands.add('confirmaReserva', () => {
  ...
  cy.get('#p-text').should('contain', 'Reserva concluída no nome de Val para o(os) dia(as) selecionado(os), no setor Setor A da Unidade B.');
...
Cypress.Commands.add('editaReserva', () => {
  ...
  cy.get('#p-text').should('contain', 'Reserva editada no nome de Val no Setor A andar para o dia 3 May 2022 na Unidade B.');
